...
You can now load `rmacrostrat` using the default `library` function, alongside some packages we will need for plotting, namely `ggplot2`, `ggrepel`, and `deeptime`:
...
From this call, we can see there is a single column named "San Juan Basin". We are also given some information about this column, such as its geographic location (in the form of a latitude and longitude), the area over which it spans, its thickness (or depth), and the number of geologic units it contains. We are also given its `col_id`, 489, which we can use to retrieve more information through other functions in `rmacrostrat`. We will do this now, specifically using `get_columns`:
...
We can see that our column spans from 2050 to 32 million years ago, from the Paleoproterozoic to the Cenozoic.
Let's take a look at the mix of lithologies contained in the column:

```{r lithologies, eval = TRUE}
# Print a summary of lithologies contained in the San Juan Basin column
San_Juan$lith
```
...
# Sort table by proportions
test <- lith_table[order(lith_table$prop)]
...
It seems the San Juan Basin is dominated by sedimentary rocks. Perhaps this is not surprising given that we know it is famed for fossils and fossil fuels!
...
Here we can see that not only does the San Juan Basin contain coal, oil, and natural gas, but also some uranium ore and aquifers.
...
So it seems over 2000 fossils are known from this single basin. We can also visualize the temporal distribution of these fossil collections, using the midpoint of their age ranges:
...
The San Juan Basin fossils range in age from the Permian to the Paleogene, but we have the highest concentration of fossil collections around the K-Pg boundary.
...
To plot the stratigraphic column, we will need to obtain data for each lithological unit contained within the San Juan Basin column. We will do this using another `rmacrostrat` function, namely `get_units`, by referencing the `column_id` for the San Juan Basin. To keep our column contained, we will limit it to geological units which are Cretaceous in age:
...
We now have information for each of the 17 Cretaceous geologic units contained within the San Juan Basin, including the age of the top and bottom of each, which is what we will use to plot our stratigraphic column.
