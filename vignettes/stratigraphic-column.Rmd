...
We can already see something that roughly resembles a geologic column. However, there is a lot we can do to improve the aesthetics of our plot. For example, the column named `color` in our dataframe specifies the hexadecimal colour corresponding to the dominant lithology of the unit. We can use this to color-code the units by lithology.
...
And finally, we can add an additional facet on the y-axis indicating the different stages of the Cretaceous, using the R package `deeptime`:
