...
```
Yup, these three are the only entries for Hell Creek. But hang on! Before going ahead and using these to pull our spatial data, you'll also notice that one of the `strat_name`s here is actually the Hell Creek "Member", which is described as belonging to the Lance Formation. Let's try getting the information again, this time specifying the stratigraphic rank of "Formation":

```{r strat_name with rank, eval = T}
# Search stratigraphic names for formations called Hell Creek
hc_def <- def_strat_names(strat_name = "hell creek", rank = "Fm")
# View data
hc_def
```
That's much better. Let's go ahead and use these `strat_name_id`s to get some spatial data. To do this, we'll need to use the `get_map_outcrop()` function, which allows us to grab outcrop polygons from geological maps. Let's do that now, specifying that we want the output to be returned as an sf object.
