...
##   Macrostrat geological database. EarthArXiv, 1–29, doi:
##   10.31223/X5XX37
## 
...
```
##   environ_id                                 name      type  class   color
## 1          1                            peritidal carbonate marine #B8B8E6
## 2          2                     shallow subtidal carbonate marine #3399FF
## 3          3                open shallow subtidal carbonate marine #3399FF
## 4          4 lagoonal/restricted shallow subtidal carbonate marine #B8B8E6
## 5          5                           sand shoal carbonate marine #5CADFF
## 6          6                                 reef carbonate marine #33CCFF
##   t_units
## 1      94
## 2     815
## 3     421
...
``` r
# Get all sedimentary Phanerozoic units
...
# Bonus plot: rock classes

We can use very similar code to explore many different geological questions. For example, what if we were interested in the temporal distribution of igneous and metamorphic rocks, as well as sedimentary? Below we extract and plot data showing exactly this.



``` r
# Get units by lithology class, interval, and project ID
...
# Load palaeoverse library
library(palaeoverse)
# Generate stage-level time bins
bins <- time_bins(interval = c("Phanerozoic"), scale = "GTS2020")
...
ggplot(counts, aes(fill = lithology_class, y = count, x = mid_ma)) +
...
<div class="figure">
<img src="class_proportions-1.png" alt="plot of chunk class_proportions" width="100%" />
<p class="caption">plot of chunk class_proportions</p>
</div>
