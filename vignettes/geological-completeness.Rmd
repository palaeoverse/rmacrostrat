...
## 
##   Jones, L.A., Dean, C.D., Gearty, W., and Allen, B.J. 2024. rmacrostrat: An R package for accessing and
##   retrieving data from the Macrostrat geological database. EarthArXiv, 1–29, doi: 10.31223/X5XX37
## 
...
Out of curiosity, let's see what types of lithologies are included in these units with unknown environments.


``` r
table(do.call(c, lapply(unknown_units$lith, function(df) df$class)))
```
...
``` r
table(do.call(c, lapply(unknown_units$lith, function(df) df$type)))
```

```
## 
##         igneous     metaigneous     metamorphic metasedimentary    metavolcanic        plutonic        volcanic 
##              51               4             484             301              19            1005            1285
```

Looks like it's all igneous and metamorphic rocks. That makes sense, it is often very difficult to assign units with lithologies of those classes to environments without more information. Let's add these unknown units to our ongoing data.frame of units for later:
